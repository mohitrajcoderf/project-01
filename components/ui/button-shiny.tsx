...
function ButtonCta({
  ...
  return (
    <Button
      variant="ghost"
      className={cn(
        "group relative w-1/2 h-12 px-4 rounded-xl overflow-hidden transition-all duration-500",
        className
      )}
      {...props}
    >
      <div className="absolute inset-0 rounded-xl p-[2px] bg-linear-to-b from-[#654358] via-[#17092A] to-[#2F0D64]">
      ...
      <div className="relative flex items-center justify-center gap-2">
        {icon && (
          <span className="text-lg bg-linear-to-b from-[#D69DDE] to-[#B873F8] bg-clip-text text-transparent drop-shadow-[0_0_12px_rgba(199,135,246,0.4)]">
            {icon}
          </span>
        )}
